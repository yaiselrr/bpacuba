    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <form id="form" role="form" method="post" enctype="multipart/form-data" action="{{route('admin.content.socials.update', $social->id)}}">
                    @csrf
                    @method('PUT')
                    <div class="box-body">
                        <div class="form-group @error('titulo') has-error @enderror">
                            ...
                            <input type="text" class="form-control" name="titulo" value="{{old('titulo',$social->titulo)}}" placeholder="Título">
                        ...
                        <div class="form-group @error('url') has-error @enderror">
                            ...
                            <input type="text"  data-model="social"  data-id="{{$social->id}}" class="form-control" name="url" value="{{old('url',$social->url)}}" placeholder="Url">
                            <span class="error-container"></span>
                            @error('url')
                            <span class="help-block" role="alert">
                                        <strong>{{ $message }}</strong>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="red_social">
                                @foreach($redes as $red)
                                    @if($social->red->id  == $red->id)
                                        <option value="{{$red->id}}" selected>{{$red->red}}</option>
                                    @else
